"""
raycast_semantic_lidar.py
-------------------------

Defines the Raycast Semantic LiDAR sensor for HoloOcean simulations.
This sensor extends the standard Raycast LiDAR by providing per-point
semantic labels, allowing identification of object classes within
...
class RaycastSemanticLidar(BaseSensor):
    """
    Raycast Semantic LiDAR sensor configuration.

    This sensor simulates a LiDAR capable of semantic segmentation,
    producing point clouds in which each 3D point carries both
    geometric (range, angle) and semantic (object label) information.
    It inherits the same physical configuration as the standard
    `RaycastLidar`, adding category annotations for machine learning
    ...
    socket : str, optional
        Data connection socket name. Default is ``"SemanticLidarSocket"``.
    ...
    ShowDebugPoints : bool, optional
        If True, enables visualization of LiDAR rays. Default is ``False``.
    ...
    **kwargs
    ...
    --------
    UserWarning
        Issued at initialization — this sensor is *Work In Progress* in HoloOcean.

    Examples
    --------
    >>> from lib.sensors.raycast_semantic_lidar import RaycastSemanticLidar
    >>> sem_lidar = RaycastSemanticLidar(Channels=16, Range=120.0)
    RaycastSemanticLidar is *Work In Progress* in HoloOcean and may not be fully supported.
    >>> print(sem_lidar.to_dict())
    {'sensor_type': 'RaycastSemanticLidar', 'socket': 'SemanticLidarSocket', 'Hz': 10,
     'configuration': {'Channels': 16, 'Range': 120.0, 'PointsPerSecond': 56000,
                       'RotationFrequency': 10.0, 'UpperFovLimit': 10.0,
    ...
    def __init__(
        self,
        Hz: int = 10,
        socket: str = "SemanticLidarSocket",
        Channels: int = 32,
        Range: float = 500.0,
        ...
        **kwargs
        ...
        warnings.warn(
            "RaycastSemanticLidar is *Work In Progress* in HoloOcean "
